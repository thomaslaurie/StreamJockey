...
const spotify = new Source({
	...
	makeAuthRequestURL(key) {
		...
		if (
			   !rules.string.test(credentials.clientId)
			|| !rules.string.test(credentials.clientSecret)
			|| !rules.string.test(credentials.redirectUri)
		) {
...
Object.assign(spotify, {
	async startAuthRequest() {
		const pack = await auth.addRequestKey();
	...
	async exchangeToken(ctx, credentials) {
		...
		//L https://developer.spotify.com/documentation/general/guides/authorization-guide/
		const result = await request('POST', 'https://accounts.spotify.com/api/token', {
		...
		const me = await session.get(ctx);
	...
	async refreshToken(ctx) {
		...
		const me = await session.get(ctx);
		const refreshToken = await User.get(me).then(one).then(resolved => resolved.spotifyRefreshToken);
