//! unfinished, don't use me yet, haven't figured out how to give databaseSocket events access to the real store? maybe sockets?, or just have the socket not included in this module
...
const store = {
	state: {
		databaseSocket = new SocketIO('/database'),
	...
	actions: {
		...
		async subscribe(context, {Entity, query, subscriber, timeout = 10000}) {
			...
			let processedQuery = await new Promise((resolve, reject) => {
				...
				context.state.databaseSocket.emit('subscribe', {table: Entity.table, query: preparedQuery}, result => {
		...
		async unsubscribe(context, {Entity, query, subscriber, timeout = 10000}) {
			...
			let processedQuery = await new Promise((resolve, reject) => {
				...
				context.state.databaseSocket.emit('unsubscribe', {table: Entity.table, query: preparedQuery}, result => {
					clearTimeout(timeoutId);
					if (sj.isType(result, sj.Error)) reject(result);
					else resolve(result);
				});
