<script>
    ...
    export default {
		...
		computed: {
			...
			usingQuery() {
				if (!sj.isSubclass(this.Entity, this.sj.Entity)) throw new this.sj.Error({
        ...
        methods: {
			async refresh(switchToState = 'delay') {
				...
				await this.refreshPromise.then(this.handleSuccess, this.handleError);
			},

			startTimeouts() {
				if (this.refreshPromise !== this.sj.Deferred) throw new this.sj.Error({
